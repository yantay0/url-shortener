...
# Prompt to enter a password for the PostgreSQL url_shortener user (rather than hard-coding
# a password in this script).
read -p "Enter password for url_shortener DB user: " DB_PASSWORD
...
# Set up the url_shortener DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE url_shortener"
sudo -i -u postgres psql -d url_shortener -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d url_shortener -c "CREATE ROLE url_shortener WITH LOGIN PASSWORD '${DB_PASSWORD}'"
sudo -i -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE url_shortener TO url_shortener;"
sudo -i -u postgres psql -c "ALTER DATABASE url_shortener OWNER TO url_shortener;"
